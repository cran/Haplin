f.slide <- function(reslist, main = ""){
##
## reslist IS A LIST OF RESULTS FROM haplin (RUN WITH use.missing = T!)
##
library(MASS)
library(mgcv) # FOR mroot

cat("MAA PASSE PAA orig.lines, BLIR I HVERT FALL FEIL DERSOM use.missing = F\n")

.l <- length(reslist)
### if(.l < 2) stop("f.slide needs more than one estimation result!")
#
##  EXTRACT SCORE MATRICES
.score <- lapply(reslist, function(x) x$score$score)
#
## EXTRACT NUMBER OF NUISANCE PARAMETERS (BETTER WAYS?)
.npars <- sapply(reslist, function(x) sum(x$info$haplos$selected.haplotypes))
cat("ad hoc!\n")
...
for (i in 1:.l){
	if(F & length(.kill[[i]]) > 0){## LITT I TVIL HER: MEN .kill BØR VEL HÅNDTERES SOM DE ANDRE NUISANCE-PARAMETRENE?
		...
		.temp <- reslist[[i]]$var.cov$var.cov.0
...
for (i in seq(along = .score)){
	tullsc <- c(tullsc, colSums(.score[[i]][,-c(1:.npars[i]), drop = F]))
}
.obs.pval.alt <- .f.test(tullsc, vcov.list = .vcov.list)

if(!.err){
	#cat("hei1\n")
	#.min.normtest1 <- apply(.normtest, 1, min)
	#cat("hei2\n")
	.min.normtest <- do.call("pmin", as.data.frame(.normtest))
	#cat("hei3\n")
	#print(all.equal(.min.normtest1, .min.normtest2))
