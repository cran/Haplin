haplin0 <- function(filename, data, pedIndex,
...
cur.call <- match.call()
.info <- f.catch0( cur.call, formals() )
#
...
cat( "--- !!! WARNING !!! ---\n
  haplin0 uses the outdated data management system based on genABEL. It will produce correct results,\n
  but will be discontinued in a future release. Please refer to the\n
  documentation on how to read and prepare data for haplin analysis with the new system.\n
  --- !!! ---\n" )
}
#
...
if( data.out == "basic" ){
	return( f.data0( data.read = .data.read, quick = T ) )
}
#
## PREPARE DATA, RETURN HERE
.tmp <- f.data0(data.read = .data.read)
#
## DATA OUT, IF REQUESTED
if( data.out == "prelim" ){
	return( f.prep.dataout( info = .tmp$info, data = .tmp$data ) )
}
#
.data <- .tmp$data
.info <- .tmp$info
#
## DISSE BLIR NAA DEFINERT BARE I f.data0 OG MAA DERFOR GJORES EKSPLISITTE:
...
.testseq.standard <- list( 
	list( maternal = F, response = "simple", x = F ),
	list( maternal = maternal, response = response, x = T )
	)
...
if( !.info$model$test.maternal & .info$model$scoretest == "only" ){
	...
	.testseq <- list(
		list( maternal = F, response = "simple", x = F ),
		list( maternal = maternal, response = response, x = T, max.EM.iter = 0, suppressEmWarnings = T )
		)
...
if( .info$model$test.maternal ){
	.testseq <- list(
		list( maternal = F, response = "simple", x = F ),
		list( maternal = F, response = response, x = T ),
		list( maternal = maternal, response = response, x = T )
		)
...
if( .test.response ){
	.testseq <- list(
		list( maternal = F, response = "simple", x = F ),
		list( maternal = maternal, response = "mult", x = T ),
		list( maternal = maternal, response = "free", x = T )
		)
...
if( scoretest == "only" ){
	if( abs( .res.0$ntri - .ntri.seq[4] ) > 1e-6 ){
		warning( "There may be a problem with the data summary", call. = FALSE )
}
}else if( max( abs( .res$ntri - .ntri.seq[4] ), abs( .res.0$ntri - .ntri.seq[4] ) ) > 1e-6 ){
	warning ( "There may be a problem with the data summary", call. = FALSE )
}
#
if( resampling == "jackknife" ){
	if( design != "triad" ){
		stop( "Jackknifing has not been tested with case-control data....", call. = F )
	}
	if( .info$model$scoretest == "only" ){
		stop( 'Jackknifing has not been tested when scoretest == "only"', call. = F )
	}
	if( xchrom ){
		stop( "Jackknifing not tested with xchrom data!", call. = F )
	}
	if( !is.null( .info$variables$covar ) ){
		stop( "Jackknifing not tested with covariates!", call. = F )
	}
