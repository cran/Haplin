pQQ <- function(pvals, nlabs = 6, conf, lim, sim = F, mark = 0.05, ...){
##
## QQ-PLOT
...
if (!missing(conf)) {
	...
	.sm.lower <- approx(x = .fix.logaux, y = -log10(.b.lower), xout = seq(.lim[1], .lim[2], length.out = 1000))
	.sm.upper <- approx(x = .fix.logaux, y = -log10(.b.upper), xout = seq(.lim[1], .lim[2], length.out = 1000))
	#
	## ADD BACKGROUND SHADING
	segments(x0 = .sm.lower$x, y0 = .sm.lower$y, x1 = .sm.upper$x, y1 = .sm.upper$y, col = "grey")
	#
	## ADD BOUNDARY LINES FOR SHADING
	lines(.fix.logaux, -log10(.b.lower), col = "black", lwd = 1)
	lines(.fix.logaux, -log10(.b.upper), col = "black", lwd = 1)
	#
	...
	if(sim){
		...
		lines(.logaux, .s.lower, lty=2, col="red", lwd=2)
		lines(.logaux, .s.upper, lty=2, col="red", lwd=2)
		#
	}
}
#
## PLOT P-VALUES
points(.logaux, .logpvals, pch = 16, cex = 0.7)
box(lwd = 2, bty = "l")
#
## LINE WITH SLOPE 1
...
if(nlabs > 0){
	...
	text(.logaux[.ind], .logpvals[.ind] - 0.05, names(.pvals)[.ind], srt = 270, font = 2, cex = 0.7, adj = 0, col = "black", xaxs = "i", yaxs = "i")
}
}
