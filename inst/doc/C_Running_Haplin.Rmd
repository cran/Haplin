...
The output of the `genDataPreprocess` function (or `genDataLoad`) is used to run the analysis.	Haplin analysis is run by the single command:
...
If we ran analysis with estimation of the maternal or POO effects, then we can choose to plot these p-values:
...
If we have data on environmental exposure of (usually) the mother, we can add it to our dataset and analyse it with `haplinStrat`, that calculates relative risks for strata defined by the categories of the environmental exposure variable.
...
The difference from running haplin is that we need to tell `haplinStrat` which column contains the environmental variable. This is done by setting the argument `strata` to a specific column number in the `gen.data$cov.data` part of data. The result of `haplinStrat` run is a list of haplin objects: one for the haplin run on the entire data, and then one for every stratum.
